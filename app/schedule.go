...
func (sc *Scheduler) Every(crontab, jobName string, payload []byte) error {
	return sc.every(crontab, hostAppNamespace, jobName, payload)
}

func (sc *Scheduler) every(crontab, namespace, jobName string, payload []byte) error {
	_, err := sc.s.NewJob(
		gocron.CronJob(crontab, false),
		gocron.NewTask(func() {
			if err := sc.app.dispatch(namespace, jobName, payload, 0); err != nil {
